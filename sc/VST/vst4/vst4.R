...
path <- dir("./raw/")
sample_names <- substr(path,12,16)
scRNAlist <- list()
for (i in 1:length(path)) {
  tmp <- Read10X_h5(paste0("./raw/",path[i]))
...
int.seu <- subset(int.seu, subset = nFeature_RNA >=250&percent.mt<20&nCount_RNA <25000)
...
int.seu <- RunUMAP(int.seu,reduction = "harmony",dims = 1:25)
int.seu = FindNeighbors(int.seu,dims=seq(25),verbose=F,reduction = "harmony")
int.seu <- FindClusters(int.seu,resolution = 0.2)
...
table(int.seu$DF.classifications_0.25_0.1_311)
DimPlot(int.seu, reduction = "umap", group.by = "DF.classifications_0.25_0.1_311")

int.seu<- subset(int.seu,subset = DF.classifications_0.25_0.05_461 =="Singlet")
qs::qsave(int.seu,"vst5.qs")
#######################
int.seu <- qs::qread("vst5.qs")
...
write.csv(cellmarker,file = "cellmarker16.csv")
