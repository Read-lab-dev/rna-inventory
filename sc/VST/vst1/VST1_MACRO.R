...
mye.seu <- subset(int.seu,final_label=="Myeloid")
...
mye.seu <- FindClusters(mye.seu,resolution = 0.3)
DimPlot(mye.seu,raster = F,label=T)
all.markers <- FindAllMarkers(mye.seu, only.pos = TRUE, min.pct = 0.25,max.cells.per.ident = 500,
                              logfc.threshold = 1) 
...
mye.seu <- subset(mye.seu,seurat_clusters%in%c(13,14),invert=T)
DimPlot(mye.seu,raster = F,label = T)

DotPlot(mye.seu,features = c("CD14","FCGR3A","CD68","CCR5","TFRC","ITGAM"))
...
qs::qsave(mye.seu,file = "./mye.seu.qs")
...
mye_sub <- read.csv("mye.csv")
mye_sub <- as.data.frame(apply(mye_sub,2, toupper))
library(UCell)
mp.genes <- as.list(mye_sub)

vst.seu <- AddModuleScore_UCell(mye.seu, features = mp.genes, ncores=4, name = "")

VlnPlot(vst.seu, features=names(mp.genes),
        pt.size = 0, ncol=5, group.by="seurat_clusters")
