...
gset <- getGEO("GSE108524", GSEMatrix =TRUE, AnnotGPL=FALSE)
...
gsms <- paste0("XXXX111111111100000000000000000")
...
ggplot(data=na.omit(resdata), aes(x=log2FoldChange, y=-log10(padj), colour=change)) +
  ...
  ggtitle("GSE108524 Sporadic vs NF2")+
  theme_clean()+
...
ggsave(filename = "GSE108524_Volcano.pdf",width  = 6,height = 4)
...
write.csv(gsea_results_df[,-c(1:2)],file = paste0("GSEA_GSE108524",".csv"))
save(exp,resdata,gsea_results,gsea_results_df,file = paste0("GSEA_GSE108524",".Rdata"))
