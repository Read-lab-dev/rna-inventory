...
gset <- getGEO("GSE141801", GSEMatrix =TRUE, AnnotGPL=FALSE)
...
gsms <- paste0("000000000000000000000000000000000000XXXXXXXXX11111",
...
resdata <- subset(tT, select=c("adj.P.Val","P.Value","t","B","logFC","Gene.Symbol"))

dat <- exprs(gset)[rownames(resdata),]
ids <- data.frame(probe_id =rownames(resdata),
                  symbol=resdata$Gene.Symbol)
dat[1:4,1:4] 
ids$median=apply(dat,1,median)
ids=ids[order(ids$symbol,ids$median,decreasing = T),]
ids=ids[!duplicated(ids$symbol),]
resdata  <- resdata[ids$probe_id,]
resdata  <- resdata[!resdata$Gene.Symbol%in%c("---",""),]
...
ggplot(data=na.omit(resdata), aes(x=log2FoldChange, y=-log10(padj), colour=change)) +
  ...
  ggtitle("GSE141801 Sporadic vs NF2")+
  theme_clean()+
...
ggsave(filename = "GSE141801_Volcano.pdf",width  = 6,height = 4)
...
write.csv(gsea_results_df[,-c(1:2)],file = paste0("GSEA_GSE141801",".csv"))
save(resdata,gsea_results,gsea_results_df,file = paste0("GSEA_GSE141801",".Rdata"))
