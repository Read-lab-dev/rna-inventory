...
dat = as.data.frame(GetAssayData(Abdelfattah2022.seu,slot='counts',assay = 'RNA'))#表达矩阵
dat[1:4,1:4]
dim(dat)

groupinfo = data.frame(v1=colnames(dat),v2=Abdelfattah2022.seu$celltype)#样本注释文件
head(groupinfo)
...
aa <- read.table("./infercnv_out/infercnv_subclusters.observation_groupings.txt",sep = " ")
aa$name <- rownames(aa)
aa$name <- gsub("_1","",aa$name)
bb <- left_join(Abdelfattah2022.seu@meta.data,aa)
View(bb)
