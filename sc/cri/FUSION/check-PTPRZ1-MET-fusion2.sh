...
for index in {1..3}; do
    ...
    samtools view  $BAM \
 chr7:116739950-122004660 | grep CCGTCTGGAAATGCGAATCCTAAA |grep CB:Z: | sed 's/.*CB:Z:\([ACGT]*\).*/\1/' | sort | uniq -c|awk 'BEGIN {OFS="\t"} {print $0, "'$SAMPLE'"}' > reads_per_barcode_${SAMPLE}_ZM.txt
done
