...
path <- grep("SCPC",dir("./raw_LGG/"),value=T)
sample_names <- paste0("CU",clipr::read_clip())
scRNAlist <- list()
...
for (i in 1:length(path)) {
  tmp <- readRDS(paste0("./raw_LGG/",path[i],"/",gsub("CS","CL",path[i]),"_filtered.rds"))
...
int.seu[["percent.mt"]] <- int.seu$subsets_mito_percent
int.seu$log10GenesPerUMI <- log10(int.seu$nFeature_RNA)/log10(int.seu$nCount_RNA)
VlnPlot(int.seu, features = c("nFeature_RNA", "nCount_RNA", "percent.mt","log10GenesPerUMI"), ncol = 4,group.by = "orig.ident")
int.seu <- subset(int.seu, subset = nFeature_RNA >=250&nCount_RNA<20000&percent.mt<25)
int.seu <- SCTransform(int.seu,vars.to.regress = c("percent.mt"))
...
FeaturePlot(HGG.seu,features = c("GFAP","OLIG1","OLIG2","MAP2","PTPRC","PDGFRA","EGFR","YAP1","WWTR1","TEAD1","TEAD4"),order = T,reduction = "umap")+
  DimPlot(int.seu,reduction = "umap",label = T,group.by = "orig.ident")
ggsave("UMAP_LGG.png",dpi = 200,height =9,width = 16)
...
write.csv(cellmarker,file = "cellmarker_LGG.csv")
features <- list(
  OPC    = c("PDGFRA","CSPG4"),
...
ggsave("2dim-HGGvsLGG.png",dpi=200,height = 3,width = 6)
