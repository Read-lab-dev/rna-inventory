...
library(Seurat)
library(dplyr)
cellchat.DMSO <- qs::qread("cellchat-DMSO.qs")
cellchat.L <- qs::qread("cellchat-VP-L.qs")
cellchat.DMSO <-subsetCellChat(cellchat.DMSO,idents.use = levels(cellchat.DMSO@idents)[-c(11,13)])
object.list <- list(DMSO=cellchat.DMSO,VP.L=cellchat.L)
...
group.cellType <- c(rep("AC", 3), rep("N",6 ),"AC",rep("Tumor", 2),"AC")
...
cellchat <- mergeCellChat(object.list, add.names = names(object.list))
gg1 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "T-OPC",comparison = c(1, 2))
gg2 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "T-cyc",comparison = c(1, 2))
patchwork::wrap_plots(plots = list(gg1,gg2))
ggsave(wrap_plots(plots = list(gg1,gg2)),file="VP.L.change.png",height = 6,width = 12)
...
ggsave(gg1+gg2,file="inforflow-VP.L.png")
...
netVisual_bubble(cellchat,sources.use = 1, targets.use = c(11:12),  comparison = c(1, 2), angle.x = 45)

gg1 <- netVisual_bubble(cellchat, sources.use = 1, targets.use = c(11:12),  comparison = c(1, 2), max.dataset = 2, title.name = "Increased signaling in VP.L", angle.x = 45, remove.isolate = T)
#> Comparing communications on a merged object
gg2 <- netVisual_bubble(cellchat, sources.use = 1, targets.use = c(11:12),  comparison = c(1, 2), max.dataset = 1, title.name = "Decreased signaling in VP.L", angle.x = 45, remove.isolate = T)
#> Comparing communications on a merged object
gg1 + gg2
...
net.up <- subsetCommunication(cellchat, net = net, datasets = "VP.L",ligand.logFC = 0.05, receptor.logFC = NULL)
# extract the ligand-receptor pairs with upregulated ligands and upregulated receptors in NL, i.e.,downregulated in LS
net.down <- subsetCommunication(cellchat, net = net, datasets = "VP.L",ligand.logFC = -0.05, receptor.logFC = NULL)
...
gg1 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.up, sources.use = 4, targets.use = c(11:12), comparison = c(1, 2),  angle.x = 90, remove.isolate = T,title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
#> Comparing communications on a merged object
pairLR.use.down = net.down[, "interaction_name", drop = F]
gg2 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.down, sources.use = 4, targets.use = c(11:12), comparison = c(1, 2),  angle.x = 90, remove.isolate = T,title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))
#> Comparing communications on a merged object
gg1 + gg2
...
par(mfrow = c(1,2), xpd=TRUE)
for (i in 1:length(object.list)) {
  netVisual_aggregate(object.list[[i]], signaling = pathways.show,targets.use=c(11,12), layout = "circle", edge.weight.max = weight.max[1], edge.width.max = 10, signaling.name = paste(pathways.show, names(object.list)[i]))
}
...
cellchat@meta$datasets = factor(cellchat@meta$datasets, levels = c("DMSO", "VP.L")) # set factor level
plotGeneExpression(cellchat, signaling = "MK", split.by = "datasets", colors.ggplot = T, type = "violin")

int.seu <- subset(int.seu,orig.ident%in%c("Eng_DMSO","Eng_VP_L"))
p2 <- VlnPlot(int.seu,features = "PTPRZ1",split.by = "orig.ident")

save(object.list, file = "cellchat_object.list_VP.L.RData")
save(cellchat, file = "cellchat_merged_VP.L.RData")
