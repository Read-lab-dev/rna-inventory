...
cellchat.S <- qs::qread("cellchat-new-S.qs")
object.list <- list(DMSO = cellchat.DMSO, VP.S = cellchat.S)
...
group.cellType <- c(rep("eGSC", 3), rep("Neuron",4 ),"AC","AC","Cycling")
...
gg2 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "tNPC-like",comparison = c(1, 2))
gg3 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "tAC-like",comparison = c(1, 2))
...
ggsave(wrap_plots(plots = list(gg1,gg2,gg3,gg4)),file="VP.S.change.pdf",height = 9,width = 12)
