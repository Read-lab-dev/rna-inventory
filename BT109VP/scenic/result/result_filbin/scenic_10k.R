...
setwd("~/rna/BT109VP/scenic/result_filbin/")
inputDir = getwd()
...
int.seu <- qs::qread("../../filbin/filbin-tumor.seu.qs")
...
# regulonActivity_byCellType_Scaled <- t(scale(t(regulonActivity_byCellType[c("TEAD4(+)","TEAD1(+)"),]
# ), center = T, scale=T))
...
int.seu$TEAD4t <- bind_data$`TEAD4(+)` 
VlnPlot(int.seu,features = "TEAD4t")
