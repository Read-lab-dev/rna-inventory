rm(list = ls())
library(Seurat)
setwd("~/rna/BT109VP/liu2022")
exp <- data.table::fread("GSE184357_counts_fresh_postFilter.txt.gz")
...
meta.data <- data.table::fread("GSE184357_metadata_fresh_postFilter.txt.gz")
dmg.seu$celltype <- meta.data$annotation
dmg.seu$age <- meta.data$age
dmg.seu$location <- meta.data$location
DimPlot(dmg.seu)
qs::qsave(dmg.seu,file = "liu2022.qs")

##############################################
###########Our Dataset#############
gsc.seu <- qs::qread("liu2022.qs")
gsc.seu <- subset(gsc.seu,celltype%in%c("Microglia","Oligodendrocyte"),invert=T)
...
ggplot(data = lineage.data,aes(x=lineage.score.plot,y=stemness,color= orig.ident))+
  geom_point(alpha=1,size=0.8)+scale_colour_manual(values = met.brewer("Klimt",18))+
  theme_few()+xlab("AC-like <--------------> NPC-like")+gghighlight(unhighlighted_params =list(colour="grey90",alpha=0.8))+
  theme(
    axis.ticks.x = element_blank(),
    axis.text.x = element_blank(),
...
ggplot(data = lineage.data,aes(x=lineage.score.plot,y=stemness,color= location))+
  geom_point(alpha=1,size=0.8)+scale_colour_manual(values = met.brewer("Klimt",3))+
  theme_few()+xlab("AC-like <--------------> NPC-like")+gghighlight(unhighlighted_params =list(colour="grey90",alpha=0.8))+
  theme(
    axis.ticks.x = element_blank(),
    axis.text.x = element_blank(),
...
ggsave("./2dim-location.pdf",height = 4,width = 8)

ggplot(data = lineage.data,aes(x=lineage.score.plot,y=stemness,color= age))+
  geom_point(alpha=1,size=0.8)+scale_colour_manual(values = met.brewer("Klimt",2))+
  theme_few()+xlab("AC-like <--------------> NPC-like")+gghighlight(unhighlighted_params =list(colour="grey90",alpha=0.8))+
  theme(
    axis.ticks.x = element_blank(),
    axis.text.x = element_blank(),
    ...
    panel.border = element_rect(fill = NA, color = "gray60", size = 1, linetype = "solid"))+
  facet_wrap(vars(age))
ggsave("./2dim-age.pdf",height = 4,width = 8)
