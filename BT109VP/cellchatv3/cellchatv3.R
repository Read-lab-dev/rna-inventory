...
DMSO <- subset(aa.seu,orig.ident=="Org")

cellchat <- createCellChat(object = DMSO, group.by = "ident", assay = "RNA")
...
# df.net <- subsetCommunication(cellchat.DMSO,targets.use = 1:3,sources.use =4:13)
qs::qsave(cellchat,file = 'cellchat-new-Org.qs')
