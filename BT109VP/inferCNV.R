...
int.seu <- subset(int.seu,celltype=="GABAN")
int.seu <- subset(int.seu,downsample=100)
gsc.seu <- subset(gsc.seu,orig.ident=="Eng_DMSO")
gsc.seu <- DietSeurat(qs::qread("./gsc.seu.qs"))
# gsc.seu$celltype <- ifelse(Idents(gsc.seu)=="tCYC","CYC",gsc.seu$celltype)
...
groupinfo = data.frame(v1=colnames(dat),v2=int.seu$celltype)#样本注释文件
head(groupinfo)
...
infercnv_obj = CreateInfercnvObject(raw_counts_matrix=exp,
                                    ...
                                    ref_group_names="GABAN")
options(scipen = 100)
infercnv_obj2 = infercnv::run(infercnv_obj,
                              cutoff=0.1, # use 1 for smart-seq, 0.1 for 10x-genomics
                              out_dir='./infercnv_out_5' ,  #选择输入文件夹
                              cluster_by_groups=T, no_plot=F,
                              cluster_references=F,
                              tumor_subcluster_partition_method="leiden",
                              analysis_mode = "subcluster",
                              hclust_method="ward.D2", plot_steps=T,num_threads = 10 ,denoise=T, HMM=T)
