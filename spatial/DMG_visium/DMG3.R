...
vis.seu <- Load10X_Spatial(data.dir = "./data/DMG3_spaceranger_out",slice ="DMG3") 
vis.seu$orig.ident <- "DMG3"
...
SpatialFeaturePlot(vis.seu, features =  c("tOPC", "tCYC","tNPC","tAC"), pt.size.factor = 1.8, ncol = 5, crop = T, alpha = c(0.1, 1))


subtype_results <- apply(GetAssayData(vis.seu,assay = "predictions"), 2, function(sample_scores) {
  names(which.max(sample_scores))
})
vis.seu$subtype <- subtype_results

SpatialDimPlot(vis.seu,group.by = "subtype",pt.size.factor = 2, crop = T,ncol = 3,cols = c("tCYC"='#999999',"tNPC"='#E69F00',"tOPC"="limegreen","tAC"="#CF5A79")) 
