...
lineage <- read.csv("../Richards/netfel30.csv",header = T,na.strings = "")
...
qs::qsave(int.seu,file = "GSE.tumor.qs")
...
ggsave("netfel_subtype.png",height = 4,width = 6)
...
ggplot(data = gsc.module,aes(x=xaxis,y=yaxis))+
  geom_point(alpha=1,size=0.1,color="grey90")+
  geom_vline(xintercept=0, linetype="dashed", colour="grey", linewidth=0.5) + 
  geom_hline(yintercept=0, linetype="dashed", colour="grey", linewidth=0.5) + 
  theme_few()+xlab("AC-like <--------------> MES-like")+ylab("AC-like <--------------> OPC-like")+
  theme(axis.ticks.x = element_blank(),
...
ggsave("Density.png",height = 3,width = 12)
DimPlot(int.seu,cells.highlight = WhichCells(int.seu,expression=RIOK2>0),
        reduction = "dim2",sizes.highlight=0.01)
FeaturePlot(int.seu,features = "TXNDC16",reduction = "dim2",order = T)
