...
library(tibble)
library(harmony)
gbm.data <- fread("./Richards/Richards_NatureCancer_GBM_scRNAseq_counts.csv.gz")
gbm.data <- column_to_rownames(gbm.data,var = "V1")
int.seu  <- CreateSeuratObject(counts = gbm.data,project = "RICHARD",min.cells=3,min.features=200)
int.seu[["percent.mt"]] <- PercentageFeatureSet(int.seu, pattern = "^MT-")
meta.data <- fread("./Richards/Richards_NatureCancer_GBM_scRNAseq_meta.csv.gz")
setwd("./Richards/")
meta.data$V1 <- gsub("-",".",meta.data$V1)
identical(colnames(int.seu),meta.data$V1)
meta.data <- column_to_rownames(meta.data,var = "V1")
int.seu@meta.data <- cbind(int.seu@meta.data,meta.data[,-c(1:4)])
...
ElbowPlot(int.seu)
int.seu <- RunHarmony(int.seu, group.by.vars = "PatientID", max_iter = 20,
...
DimPlot(int.seu,label = T)+DotPlot(int.seu, features = genes_to_check,assay='RNA')  
cellmarker <- NULL
...
seurat_colors <- sample(MetBrewer::met.brewer("Klimt",7),5)
...
qs::qsave(int.seu,file = "RICHARD.qs")
ggsave("TXNDC16.png",dpi = 200,height = 4,width = 5)
